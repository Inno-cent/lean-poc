...
const UpdateContact = () => {
  ...
  const { contact, setRefreshContacts } = route.params;
  ...
  const [countryCode, setCountryCode] = useState(contact.international_dialing_code || '');
  const navigation = useNavigation();
    ...
    if (contact) {
      ...
      setCountryCode(contact.international_dialing_code);
    }
  }, [contact]);
  ...
  const displayMessage = (msg: string) => {
    ...
    setLoading(true);
    try {
      await updateContact(contact._id, countryCode, phoneNumber, lastName, firstName, displayMessage, setIsSuccess, setLoading, navigation, setRefreshContacts); // Pass setRefreshContacts
      setIsSuccess(true);
    } catch (error) {
